...
func (set *IPSet) Validate() bool {
	...
	if set.SetType == HashIP || set.SetType == HashIPPort || set.SetType == HashIPPortIP || set.SetType == HashIPPortNet {
...
func (e *Entry) String() string {
	switch e.SetType {
	case HashIP:
		// Entry{192.168.1.1, udp} -> 192.168.1.1,udp
		// Entry{192.168.1.2, tcp} -> 192.168.1.2,tcp
		return fmt.Sprintf("%s,%s", e.IP, e.Protocol)
	case HashIPPort:
...
func (runner *runner) createSet(set *IPSet, ignoreExistErr bool) error {
	...
	if set.SetType == HashIP || set.SetType == HashIPPortIP || set.SetType == HashIPPort || set.SetType == HashIPPortNet {
		args = append(args,
...
func (runner *runner) TestEntry(entry string, set string) (bool, error) {
	if out, err := runner.exec.Command(IPSetCmd, "test", set, entry).CombinedOutput(); err == nil {
		...
		// 看起来是麒麟arm64的问题
		reg, e := regexp.Compile("is NOT in set " + set)
		if e == nil && reg.MatchString(err.Error()) {
			return false, nil
		}
